...
@router.get("/debug/deep-dive")
async def deep_dive_embedding_issue(
    db: AsyncSession = Depends(get_db_session)
):
    """Deep dive into the NLP project embedding issue"""
    
    # Get the sentiment analysis project (ID 6)
    project_result = await db.execute(
        select(projects).where(projects.c.id == 6)
    )
    project = project_result.fetchone()
    
    if not project:
        return {"error": "Project 6 not found"}
    
    # Get its embedding
    embedding_result = await db.execute(
        select(project_embeddings).where(project_embeddings.c.project_id == 6)
    )
    embedding_row = embedding_result.fetchone()
    
    # What text was used to generate this embedding?
    original_embedding_text = f"{project.title}. {project.description}. "
    original_embedding_text += f"Topics: {', '.join(project.topics)}. "
    original_embedding_text += f"Language: {project.language}. "
    original_embedding_text += f"Difficulty: {project.difficulty}"
    ...
    for query in queries:
        ...
        if embedding_row and embedding_row.embedding:
            project_emb = np.array(embedding_row.embedding)
            query_emb = np.array(query_embedding)
    ...
    return {
        ...
        "embedding_text_used": original_embedding_text,
        "has_embedding": embedding_row is not None,
        "embedding_length": len(embedding_row.embedding) if embedding_row and embedding_row.embedding else 0,
        "query_similarity_scores": results
    }